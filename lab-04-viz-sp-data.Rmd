...
There are `r nrow(dennys)` rows and `r ncol(dennys)` columns. Each row represents the data for one dennys restaurant. The variables are:
`r names(dennys)`.
...
There are `r nrow(laquinta)` rows and `r ncol(laquinta)` columns. Each row represents the data for one La Quintas Inns The variables are:
`r names(laquinta)`.
...
***There are La Quinta locations outside the US but they are mostly located in America. They're located in Mexico, Canada, China, New Zealand, Turkey, Ecuador, etc.
Dennys is only located in the US. ***
...
***We can use the State variable to determine whether a location is in the US. Specifically by comparing the state listed to a database of states in the US.***
...
```
***There are no dennys locations outside the US in our data.***
...
dennys %>%
  ...
  arrange(desc(n))
```
***California has the most Dennys locations while Delaware has the least. This isn't surprising because of the popularity of the states and the size also.*** 
...
laquinta %>%
  ...
  arrange(desc(n))
```
***Texas has the most La Quinta locations while Maine has the least. This isn't surprising because of the population of the states and the size also.***
...
***District of Columbia has the most Dennys locations while Rhode Island has the most La Quinta locations.***
